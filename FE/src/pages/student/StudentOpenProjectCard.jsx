...
function getTypeMeta(projectType) {
  if (projectType === "PRIVATE") {
    return {
      label: "Riêng tư",
      tone: "border-d4u-cyan/20 bg-d4u-soft/65 text-d4u-teal-deep",
    };
  }
...
export function StudentOpenProjectCard({ project, onOpen }) {
  const typeMeta = getTypeMeta(project.projectType);
  const primaryLabel = project.hasApplied ? "Đã ứng tuyển" : "Xem chi tiết";
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-[34px] flex-wrap items-center gap-2 pt-1">
        ...
        {project.hasApplied ? (
          <Tag className="!m-0 !rounded-full !border-blue-200 !bg-blue-50 !px-3 !py-1 !font-semibold !text-blue-700">
            Đã ứng tuyển
          </Tag>
        ) : null}
        ...
        <div className="grid min-h-[168px] content-start gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-d4u-text-3">
            <span className="rounded-full border border-d4u-border/70 bg-d4u-soft/45 px-3 py-1">
              Đọc full brief trong trang chi tiết dự án
            </span>
          </div>
        </div>
