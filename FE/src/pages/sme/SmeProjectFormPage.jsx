      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl space-y-3">
          <span className="inline-flex rounded-chip bg-white/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-d4u-teal-deep/80">
            {deadlineOnly ? 'Điều chỉnh deadline' : 'Tạo dự án'}
          </span>
  ...
  return (
    <section className="rounded-panel border border-d4u-border bg-d4u-surface p-5 shadow-soft">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-base font-semibold text-d4u-text-1">Tóm tắt timeline</h2>
      ...
      {deadlineLocked ? (
        <Alert
          ...
          type="error"
          showIcon
          message="Deadline của dự án này đã bị khóa vì đã có đề nghị hợp tác hoặc dự án đã vượt qua giai đoạn cho phép chỉnh sửa."
        />
      ) : null}
    </section>
  );
...
export function SmeProjectFormPage({ mode }) {
  ...
  const applyAiSuggestionToForm = () => {
    ...
    message.success('Đã áp dụng gợi ý AI vào biểu mẫu. SME cần kiểm tra lại trước khi lưu hoặc đăng dự án.');
  };
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <section className="rounded-panel border border-d4u-border bg-d4u-surface p-6 shadow-soft">
          ...
          <Alert
            ...
            type="info"
            showIcon
            message="AI chỉ gợi ý nội dung. SME vẫn là bên quyết định brief, ngân sách, deadline và trạng thái đăng cuối cùng."
          />
