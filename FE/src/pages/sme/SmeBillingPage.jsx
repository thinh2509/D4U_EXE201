...
function getSmeBudgetLimit(profile) {
  ...
  if (maxBudget == null) return 'Không giới hạn ngân sách';
  return `Tối đa ${formatCurrency(maxBudget, 'VND')} / dự án`;
}
...
function getSmeSummaryDescription(profile, activeEntitlement, latestPurchase) {
  if (activeEntitlement) {
    return `Gói hiện có hiệu lực đến ${formatDate(activeEntitlement.expiresAt)}. Bạn có thể dùng AI Matching và mở rộng giới hạn dự án đang mở ngay trong khu vực làm việc SME.`;
  }
  ...
  if (profile?.isFreePlan) {
    return `Bạn đang ở gói Free với giới hạn cơ bản: ${getSmeProjectLimit(profile)} và ${getSmeBudgetLimit(profile)}. Nâng cấp để mở AI Matching và vận hành dự án có ngân sách cao hơn.`;
  }
  return 'Mua gói SME AI để mở AI Matching, tăng giới hạn dự án đang mở và vận hành quy trình tuyển chọn rõ ràng hơn.';
}
...
function SmeBillingPlanCard({
  ...
  return (
    <BillingPlanCard
      status={status}
      audienceLabel="Gói tăng trưởng SME"
      title={featuredPackage.name || 'Gói SME AI 30 ngày'}
      ...
      metrics={[
        ...
        { icon: <FileSearchOutlined />, label: 'Giới hạn dự án', value: `${featuredPackage.maxActiveOpenProjectsOverride || 10} dự án` },
...
export function SmeBillingPage() {
  ...
  const startPurchase = async (pkg) => {
    ...
    try {
      ...
      message.success('Đã tạo giao dịch thanh toán cho gói SME AI.');
      await loadData();
    } catch (requestError) {
    ...
    <>
      ...
      {!activeEntitlement && latestPurchase?.paymentStatus !== 'PENDING' ? (
        <BillingInfoAlert
          ...
          description={`Bạn có thể tiếp tục dùng gói Free với giới hạn cơ bản: ${getSmeProjectLimit(profile)} và ${getSmeBudgetLimit(profile)}. Nâng cấp để mở AI Matching và tăng khả năng đăng dự án.`}
        />
      ) : null}
      ...
      {profile?.isFreePlan && !activeEntitlement ? (
        <div className="mt-4">
          <BillingPill tone="neutral">Gói Free hiện tại phù hợp để bắt đầu: tối đa 2 dự án đang mở và ngân sách 5.000.000đ cho mỗi dự án.</BillingPill>
        </div>
      ) : null}
