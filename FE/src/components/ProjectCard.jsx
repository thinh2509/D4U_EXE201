...
export function ProjectCard({ project, onOpen, actionLabel = 'Xem chi tiáº¿t' }) {
  return (
    <article
    ...
    >
      ...
      <div className="flex min-h-[32px] flex-wrap items-center gap-2 pt-1">
        ...
        {project.hasApplied ? <Tag className="!m-0 rounded-full font-semibold" color="blue">ÄÃ£ á»©ng tuyá»ƒn</Tag> : null}
        {project.designCategoryName ? <Tag className="!m-0 rounded-full font-semibold" color="cyan">{project.designCategoryName}</Tag> : null}
      </div>
      ...
      <div className="mt-4 grid flex-1 gap-3">
        ...
        <div className="mt-auto grid gap-3 border-t border-d4u-border/70 pt-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <span className="flex items-start gap-3 rounded-card bg-d4u-soft/80 px-3 py-3">
              ...
              <span className="grid gap-1">
                <small className="text-[11px] font-bold uppercase tracking-[0.04em] text-d4u-text-3">NgÃ¢n sÃ¡ch</small>
                <strong className="text-sm leading-tight text-d4u-text-1">{formatCurrency(project.budgetAmount, project.currency)}</strong>
              </span>
            </span>
            <span className="flex items-start gap-3 rounded-card bg-d4u-soft/80 px-3 py-3">
              <CalendarOutlined className="mt-0.5 text-d4u-cyan" />
              <span className="grid gap-1">
                <small className="text-[11px] font-bold uppercase tracking-[0.04em] text-d4u-text-3">HoÃ n táº¥t review</small>
                <strong className="text-sm leading-tight text-d4u-text-1">{formatDate(project.totalDeadlineAt)}</strong>
              </span>
            </span>
          </div>
          ...
          <Button
            ...
            type="primary"
            ghost
            icon={actionLabel === 'Xem chi tiáº¿t' ? <EyeOutlined /> : <FileTextOutlined />}
            onClick={(event) => {
              event.stopPropagation();
              onOpen();
            }}
          >
            {project.hasApplied ? 'ÄÃ£ á»©ng tuyá»ƒn' : actionLabel}
          </Button>
        </div>
      </div>
